...
var invoice = new Animation(
  ...
  function(e) {
    ...
    this.element.style.backgroundPosition = "0px "+Math.max(t/2*-1,0)+"px";
    invoiceStamp.style.webkitTransform = "scale("+Math.max(1+t/200*-1,1)+")";
    invoiceStamp.style.opacity = Math.min(1-t/300*-1,1)
  },
  function() {
...
var report = new Animation(
  document.getElementById("reports"),
  function(e) {
    var t = -1 * this.height + (e-this.start);
    reportsTooltip.style.opacity = Math.max(Math.min(t/300,1),0);
    reportsTooltip.style.webkitTransform = "scale("+Math.max(Math.min(t/300,1),0)+")"
  },
  function() {},
...
var iphone = new Animation(
  document.getElementById("iphone"),
  function(e) {
    var t = -1 * this.height + (e-this.start)
    n = Math.max(t/2*-1,0)+"px 0px";
